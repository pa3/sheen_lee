...
public class Boss4DeathState implements State {
    ...
    public function Boss4DeathState(me:AIPlayer) {
        reset(me);
    }

    public static function newInstance(me:AIPlayer):Boss4DeathState {

        if (_pool.length > 0) {
            var result:Boss4DeathState = _pool.shift();
            result.reset(me);
            return result;
        } else {
            return new Boss4DeathState(me);
        }
    }

    public static function release(state:Boss4DeathState):void {
        _pool.push(state);
    }
