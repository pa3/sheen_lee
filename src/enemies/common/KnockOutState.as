...
public class KnockOutState implements State {
    ...
    public function KnockOutState(me:AIPlayer) {
        reset(me);
    }

    public static function newInstance(me:AIPlayer):KnockOutState {

        if (_pool.length > 0) {
            var result:KnockOutState = _pool.shift();
            result.reset(me);
            return result;
        } else {
            return new KnockOutState(me);
        }
    }

    public static function release(state:KnockOutState):void {
        _pool.push(state);
    }
    ...
    public function enter():void {
        if (_me.health <= 0) {
            _me.fsm.changeState(DeathState.newInstance(_me));
            return;
        }
